...
public class AlertStatusService : IAlertStatusService
{
    ...
    private readonly SensorsReadDbContext _sensorsDb;

    public AlertStatusService(AlertsDbContext alertsDb, SensorsReadDbContext sensorsDb)
    {
        _alertsDb = alertsDb;
        _sensorsDb = sensorsDb;
    }
    ...
    public async Task<DashboardResponse> GetDashboardAsync(Guid talhaoId, DateTime? start, DateTime? end)
    {
        ...
        var query = _sensorsDb.SensorData.AsNoTracking()
